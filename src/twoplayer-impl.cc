...
TwoPlayerMode::TwoPlayerMode(const ProcessedInput &input) : 
    ...
    players.emplace_back(make_unique<Player>(PLAYER1, board.get(), ability_orders[0])); // Is .get() here fine??? It means every copied ptr of Board is a raw ptr!!!!!
    players.emplace_back(make_unique<Player>(PLAYER2, board.get(), ability_orders[1]));
...
PlayerID TwoPlayerMode::runGame() {
    ...
    unique_ptr<Player> current_player_ptr = players[0];
